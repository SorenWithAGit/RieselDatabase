    ...
    def read_hr_weather(filepath: str):
        ...
        or len(avg_rows[2]) == 16:
            ...
            for prow in prev_rows:
                prow.append("before check")

            prev_df = pd.DataFrame(prev_rows, columns = columns1_check)
            prev_df.index = prev_line_n
            # print(prev_df)
            # print("\n")

            for crow in checkrows:
                if "before check" in crow:
                    crow.remove("before check")
                crow.append("check")
                # print(len(crow))
                # print(crow)

            check_df = pd.DataFrame(checkrows, columns = columns1_check)
            check_df.index = check_line_n
            # print(check_df)
            # print("\n")

            for nrow in next_rows:
                if "before check" in nrow:
                    nrow.remove("before check")
                if "check" in nrow:
                    nrow.remove("check")
                nrow.append("after check")
            next_df = pd.DataFrame(next_rows, columns = columns1_check)
            ...
            if filepath == r"I:\programming\python\riesel_file_checker\Harmel\Hourly weather\2003ries.txt":
                correction_df = pd.read_excel(r"I:\programming\python\riesel_DB\2003ries_check.xlsx", 
                                              sheet_name = "2003ries_fixes",
                                              index_col = 0)
