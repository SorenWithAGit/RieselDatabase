...
class runoff_calculator():
        ...
        def create_flow_calculator(self, site, df):
                ...
                # Identify and use constants baed on site
                t_interval = self.field_constants["sampling interval"][site]
                multiplier_list = self.field_constants["flow constants"][site]
                check_list = self.field_constants["flow checks"][site]
                active_checks = check_list[:-1]
        ...
        def solve_flow_constants(self, levels, flow):
                solutions = []
                equations = []
                for l, level in enumerate(levels):
                        m, e = sym.symbols("m e")
                        eq = sym.Eq(m * level**e, flow[l])
                        equations.append(eq)
                solution = sym.solve(equations, (m, e), dict = True)
                solutions.append(solution)
                return solutions


c = runoff_calculator()
sol = c.solve_flow_constants([.655, .635], [12.6226, 12.0201])
print(sol)